/*! openleg - v - 2016-10-31 *//** --- Module configuration --- */
 ...
 */
coreModule.filter("ordinalSuffix",["$filter",function($filter){var suffixes=["th","st","nd","rd"];return function(input){if("number"==typeof input&&input%1===0){var relevantDigits=20>input?input%20:input%10;return input.toString().concat(3>=relevantDigits?suffixes[relevantDigits]:suffixes[0])}return"D:"}}]),/**
...
for(var month=0;12>month;month++)if(!foundMonths.hasOwnProperty(month)){var momentDate=moment([years[i],month,1]);$scope.events.push({title:"No sessions exist for the month of "+momentDate.format("MMMM")+".",start:momentDate.day("Monday").add(7,"days").format("YYYY-MM-DD"),
